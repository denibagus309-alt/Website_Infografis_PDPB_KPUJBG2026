...
async function loadFromGoogleSheets() {
  ...
  for (const [key, candidates] of Object.entries(SHEET_TAB_CANDIDATES)) {
    try {
      ...
      const headerIdx = findHeaderRowIndex(found.rows);
      const headerRow = headerIdx !== -1 ? found.rows[headerIdx] : null;

      let parsed = null;
      if (key === "rekap") parsed = extractRekap(found.rows);
      else if (key === "usia") parsed = extractUsia(found.rows);
      else if (key === "beritaAcara") parsed = extractBeritaAcara(found.rows);
      else parsed = extractMutasi(found.rows);

      if (parsed) {
        if (key === "rekap") rawRekap = parsed;
        else if (key === "usia") rawUsia = parsed;
        else if (key === "meninggal") rawMeninggal = parsed;
        else if (key === "pindah") rawPindah = parsed;
        else if (key === "beritaAcara") rawBeritaAcara = parsed;
        successCount++;
        diagnostics[key] = { status: "ok", tabName: found.tabName, headerRow, rowCount: parsed.length };
      } else {
        failedTabs.push(found.tabName);
        diagnostics[key] = {
          status: headerRow ? "header_not_recognized" : "no_header_row",
      ...
      diagnostics[key] = { status: "error", message: String(err) };
    }
  }
